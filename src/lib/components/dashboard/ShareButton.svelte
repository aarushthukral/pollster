    ...
    <PopoverPanel
      ...
      <div class="overflow-hidden rounded-lg border border-gray-200 bg-white px-6 py-4 shadow-lg">
        ...
        <div class="mt-1 flex h-8 w-full">
          <input
            type="text"
            class="flex-grow rounded-l-lg border border-gray-300 px-2"
            readonly={true}
            bind:this={inputBox}
          ...
          <button
            ...
            class="flex h-8 w-8 flex-shrink-0 cursor-pointer items-center justify-center rounded-r-lg border border-l-0 border-gray-300 bg-gray-200 p-1">
            {#if !copied}
              <ClipboardCopyIcon />
            {:else}
              <CheckIcon />
